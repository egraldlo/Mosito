...
TaskThreadPool::TaskThreadPool(int nthreads,int capacity)
:nthreads_(nthreads),queue_(new CircleQueue<TaskInfo>(capacity)) {
...
void *TaskThreadPool::single_thread(void *args) {
	TaskThreadPool *tp=reinterpret_cast<TaskThreadPool *>(args);
	while(true){
		pthread_mutex_lock(&(tp->lock_));
		/*TODO: "IF" will let two ways go, but "WHILE" won't */
		while(tp->queue_->empty()==true){
...
void TaskThreadPool::init_thread_pool() {
	pthread_mutex_init(&lock_,0);
	pthread_cond_init(&condition_,0);
	pthread_t *threads_array=new pthread_t[nthreads_];
	for(unsigned i=0;i<nthreads_;i++){
...
void TaskThreadPool::destroy_thread_pool() {
	/* TODO: how to terminate the single thread. */
	pthread_cond_destroy(&condition_);
	pthread_mutex_destroy(&lock_);
}

void TaskThreadPool::add_into_thread_pool(TaskInfo *task) {
	pthread_mutex_lock(&lock_);
	while(true){
		if(queue_->push(task)==true){
			pthread_cond_signal(&condition_);
